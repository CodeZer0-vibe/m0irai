...
pub struct AgentMessageBlock {
    ...
    /// Optional caller-owned rail hue for service-neutral speaker identity.
    accent_color: Option<Color>,
    /// FL-141: when set, every rendered span's foreground is folded to this
    /// colour. The rail (`accent_color`) is a separate axis and stays where
    /// the caller put it - a block can be gray-railed and full-colour-texted,
    /// which is exactly the state the first FL-141 fix shipped by accident.
    dim_text: Option<Color>,
}
...
impl AgentMessageBlock {
    ...
    /// Create a complete message with a stable caller-owned accent rail.
    pub fn with_accent(text: impl Into<String>, accent_color: Color) -> Self {
        let mut block = Self::new(text);
        block.accent_color = Some(accent_color);
        block
    }

    /// FL-141: a complete message whose TEXT is painted `dim_text`, not just
    /// its rail.
    ///
    /// Upstream's precedent for a cancelled block is `theme.dim()` applied to
    /// the text spans as they are built -
    /// `D:/grok-ref/crates/codegen/xai-grok-pager/src/scrollback/blocks/workflow.rs:109-115`
    /// (`spans.push(Span::styled(verb, text_style))`). Deviation, stated: a
    /// workflow block owns every span it emits and can choose the style before
    /// building them; this block does not - `MarkdownContent` owns the spans -
    /// so the same colour is folded over the rendered output instead. The
    /// observable is identical (every glyph of the row paints `dim_text`);
    /// only the seam differs.
    pub fn dimmed_with_accent(
    ...
    /// Create a streaming message whose native rail remains tied to its
    /// service-neutral speaker rather than to an inferred provider state.
    pub fn streaming_with_accent(accent_color: Color) -> Self {
    ...
    /// The optional native entry-rail color supplied by the presentation
    /// owner. `None` preserves normal single-agent pager behavior.
...
/// FL-141: repaint every span's foreground, leaving text, wrapping, selection
/// ranges and backgrounds exactly as the renderer produced them.
///
/// Foreground only, deliberately. A code fence's fill and a diff row's shading
/// are semantic backgrounds; dropping them would change what the row MEANS,
/// while the ruling only asked for the row to read gray.
///
/// Dimming a markdown body INCLUDING its code fences follows upstream, which
/// does the same for a thinking block: every wrapped markdown line goes through
/// `blend_line_with_default`
/// (`D:/grok-ref/crates/codegen/xai-grok-pager/src/scrollback/blocks/thinking.rs:296-301`,
/// `D:/grok-ref/crates/codegen/xai-grok-pager-render/src/render/color.rs:245-263`),
/// fences included.
///
/// DEVIATION from upstream, stated: upstream BLENDS each span's own colour
/// toward a base by an opacity factor; this FLATTENS every span to one gray.
/// Flat is what the operator's ruling asked for ("the 3 from top should become
/// gray") and it is the only version a per-cell assertion can pin — a blend
/// leaves every cell a slightly different colour and there is nothing exact
/// left to test.
///
/// COST, measured on a painted frame rather than reasoned: inside a code fence
/// the result is `Rgb(90,98,116)` on `Rgb(44,44,44)`, a WCAG contrast ratio of
/// 2.28:1, where the same gray on the terminal background is 3.43:1. Nothing
/// becomes invisible — `a_cancelled_lanes_code_fence_never_paints_text_onto_its_own_colour`
/// gates that — but a cancelled lane's code output is harder to read than its
/// prose, and the operator has been told so.
fn dim_spans(mut out: BlockOutput, dim: Color) -> BlockOutput {
...
impl BlockContent for AgentMessageBlock {
    fn output(&self, ctx: &BlockContext) -> BlockOutput {
        ...
        let out = if ctx.raw || self.mermaid.is_empty() {
            self.content.output(ctx.width as usize)
        } else {
            self.rendered_output(ctx).0
        };
        // FL-141: last, and over everything. A cancelled lane's row is gray as
        // a whole - the answer it had already streamed included - so the fold
        // has to sit after markdown has finished choosing per-token hues, not
        // beside them.
        match self.dim_text {
            Some(dim) => dim_spans(out, dim),
            None => out,
        }
    }
    ...
    fn accent(&self, ctx: &BlockContext) -> Option<AccentStyle> {
        self.accent_color.map(|color| {
            if ctx.is_running {
                AccentStyle::animated(color)
            } else {
                AccentStyle::static_color(color)
            }
        })
    }
