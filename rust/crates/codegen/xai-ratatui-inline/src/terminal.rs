    ...
    B: Backend,
{
    ...
    /// Invalidates every cell in the inactive comparison buffer so the next
    /// flush rewrites the complete viewport without clearing the physical
    /// terminal first.
    ///
    /// Unlike [`Self::reset_back_buffer`], this is appropriate when pixels on
    /// the physical terminal may be stale: resetting to ordinary blank cells
    /// would make a blank target compare equal and leave those pixels behind.
    pub fn invalidate_back_buffer(&mut self) {
        const REDRAW_SENTINEL: &str = "\0";
        for cell in &mut self.buffers[1 - self.current].content {
            cell.set_symbol(REDRAW_SENTINEL);
        }
        self.reset_back_links();
    }
