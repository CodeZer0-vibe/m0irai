...
mod tests {
    ...
    fn tool_path_file_target_resolves_relative_against_cwd() {
        ...
        assert_file_target_and_native_url(&target, &expected);
    }

    #[test]
    fn tool_path_file_target_accepts_absolute_without_existing_file() {
        let dir = tempfile::tempdir().unwrap();
        let expected = dir.path().join("does-not-exist-xyz/foo.rs");
        let raw = expected.to_string_lossy();
        let target = tool_path_file_target(&raw, None).expect("target");
        assert_file_target_and_native_url(&target, &expected);
    }
    ...
    fn tool_path_file_target_preserves_parent_segments_for_os_resolution() {
        let dir = tempfile::tempdir().unwrap();
        let raw_path = dir.path().join("repo/link/../target.rs");
        ...
        assert_eq!(file_path_to_url(&path), expected_file_url(&raw_path));
    }
    ...
    fn resolve_target_keeps_standard_scheme_filter_and_file_open_path() {
        ...
        let dir = tempfile::tempdir().unwrap();
        let file_path = dir.path().join("a b.rs");
        let file = LinkTarget::File(Arc::from(file_path.as_path()));
        assert_eq!(
            resolve_link_target(&file).unwrap(),
            ResolvedLinkTarget {
                osc8_url: expected_file_url(&file_path),
    ...
    fn official_vscode_remote_file_delegation_is_exact() {
        ...
        struct Case {
            ...
            target: LinkTarget,
            presentation: LinkPresentation,
            expected_osc8: Option<Arc<str>>,
            expected_open: bool,
        }

        let dir = tempfile::tempdir().unwrap();
        let file_path = dir.path().join("worktree/src/main.rs");
        let file = LinkTarget::File(Arc::from(file_path.as_path()));
        let native_file_url = expected_file_url(&file_path);
        ...
        let cases = [
            Case {
                ...
                target: file.clone(),
                presentation: LinkPresentation::SelfResolvingPath,
                expected_osc8: native_file_url.clone(),
                expected_open: true,
            },
            Case {
            ...
            Case {
                ...
                target: web,
                presentation: LinkPresentation::Opaque,
                expected_osc8: Some(Arc::from("https://example.com/docs")),
                expected_open: true,
            },
            Case {
    ...
    fn expected_file_url(path: &Path) -> Option<Arc<str>> {
        url::Url::from_file_path(path)
            .ok()
            .map(|url| Arc::<str>::from(url.as_str()))
    }

    fn assert_file_target_and_native_url(target: &LinkTarget, expected_path: &Path) {
        let LinkTarget::File(actual_path) = target else {
            panic!("expected file target, got {target:?}");
        };
        assert_eq!(actual_path.as_ref(), expected_path);
        assert_eq!(
            resolve_link_target(target).and_then(|resolved| resolved.osc8_url),
            expected_file_url(expected_path)
        );
    }
    ...
    fn scan_detects_absolute_file_path() {
        let path = "/Users/foo/src/main.rs";
        let line = make_line(&format!("Error in {path} at line 10"));
        let mut overlay = LinkOverlay::new();
        scan_unjoined(std::iter::once((0, &line)), 0, &[], &mut overlay);

        assert_eq!(overlay.links().len(), 1);
        assert_file_target_and_native_url(&overlay.links()[0].target, Path::new(path));
    }
    ...
    fn scan_detects_grok_session_media_path() {
        ...
        let path = "/Users/alice/.grok/sessions/%2Fabc/00000000/images/1.jpg";
        let line = make_line(&format!("Saved to {path}."));
        let mut overlay = LinkOverlay::new();
        scan_unjoined(std::iter::once((0, &line)), 0, &[], &mut overlay);
    ...
    fn scan_detects_media_path_soft_wrapped_across_rows() {
        ...
        let path = concat!(
            "/Users/alice/.grok/sessions/%2FUsers%2Fali",
        ...
        for link in overlay.links() {
            assert_file_target_and_native_url(&link.target, Path::new(path));
        }
    ...
    fn scan_wrapped_path_trailing_sentence_period_excluded() {
        ...
        let path = "/Users/me/.grok/sessions/%2Fabc/019f3a86/images/1.jpg";
    ...
    fn scan_path_split_across_styled_spans_single_row() {
        let path = "/Users/foo/images/1.jpg";
    ...
    fn scan_file_path_stops_at_colon() {
        let path = "/Users/foo/bar.rs";
        let line = make_line(&format!("{path}:45:10: error message"));
        let mut overlay = LinkOverlay::new();
        scan_unjoined(std::iter::once((0, &line)), 0, &[], &mut overlay);
    ...
    fn scan_url_and_file_path_coexist() {
        let path = "/Users/me/src/lib.rs";
        let line = make_line(&format!("See https://docs.rs/foo and {path} end."));
        let mut overlay = LinkOverlay::new();
        scan_unjoined(std::iter::once((0, &line)), 0, &[], &mut overlay);
        ...
        let web = overlay
            .links()
            .iter()
            .find(|link| matches!(&link.target, LinkTarget::Url(url) if url.as_ref() == "https://docs.rs/foo"))
            .expect("web link");
        assert_eq!(
            resolve_link_target(&web.target)
                .and_then(|resolved| resolved.osc8_url)
                .as_deref(),
            Some("https://docs.rs/foo")
        );
        let file = overlay
            .links()
            .iter()
            .find(|link| matches!(link.target, LinkTarget::File(_)))
            .expect("file link");
        assert_file_target_and_native_url(&file.target, Path::new(path));
    }

    #[test]
    fn scan_file_path_with_dots_and_hyphens() {
        let path = "/tmp/grok-impl-summary.md";
        let line = make_line(&format!("Reading {path} now."));
        let mut overlay = LinkOverlay::new();
        scan_unjoined(std::iter::once((0, &line)), 0, &[], &mut overlay);
    ...
    fn scan_file_path_with_at_sign() {
        ...
        let line = make_line(&format!("In {path} now."));
        let mut overlay = LinkOverlay::new();
        scan_unjoined(std::iter::once((0, &line)), 0, &[], &mut overlay);
    ...
    fn scan_file_path_space_does_not_swallow_trailing_sentence() {
        ...
        let line = make_line(&format!("See {path} here."));
        let mut overlay = LinkOverlay::new();
        scan_unjoined(std::iter::once((0, &line)), 0, &[], &mut overlay);

        assert_eq!(overlay.links().len(), 1);
        assert_file_target_and_native_url(&overlay.links()[0].target, Path::new(path));
        // "See " = 4 cols; the link must stop before the trailing prose.
        assert_eq!(overlay.links()[0].col_start, 4);
        assert_eq!(
            overlay.links()[0].col_end,
            4 + UnicodeWidthStr::width(path) as u16
        );
    }
