...
mod grok {
    ...
    mod tests {
        ...
        fn entry(block: ToolCallBlock) -> ScrollbackEntry {
            ScrollbackEntry::new(RenderBlock::ToolCall(block))
        ...
        fn truncation_label_skips_hidden_thinking_without_consuming_limit() {
            ...
            let l =
                truncation_header_label(&refs, 0..refs.len(), Some(2), false, &Theme::current())
                    .expect("buckets");
            assert_eq!(l.text, "Ran 2 commands");
        }
...
#[cfg(feature = "grok-runtime")]
pub use grok::{
    GroupHeaderLabel, VerbGroupHeaderLabel, truncation_header_label, verb_group_header_label,
};
#[cfg(feature = "grok-runtime")]
pub(crate) use grok::{RunScan, RunStep, run_step, scan_run_forward, verb_group_kind_changed};

// Room rendering retains the shared layout and fold state machine, but service
// action rows do not exist in this runtime. A thought can still be transparent
// to an enclosing run; it cannot create a service verb group of its own.
...
mod frontend {
    ...
    pub(crate) fn verb_group_kind_changed(_old: &RenderBlock, _new: &RenderBlock) -> bool {
        false
    }
    ...
    impl GroupHeaderLabel {
        pub fn label(&self) -> &VerbGroupHeaderLabel {
            match self {
                Self::VerbRun(label) | Self::Truncation(label) => label,
            }
        }
    }

    pub fn verb_group_header_label(
        _entries: &[&ScrollbackEntry],
        _header_idx: usize,
        _end: usize,
        _show_thinking: bool,
        _theme: &Theme,
...
#[cfg(not(feature = "grok-runtime"))]
pub(crate) use frontend::{RunScan, RunStep, run_step, scan_run_forward, verb_group_kind_changed};
