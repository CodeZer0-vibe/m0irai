...
pub(super) enum VersionBadgeMode<'a> {
    /// Full badge: team | tier | api_key | **m0irai** VERSION+channel (right-aligned).
    Full { subscription_tier: Option<&'a str> },
    /// Hero footer: team | api_key | channel (right-aligned, gray).
    HeroFooter,
    /// Hero inline: **m0irai**  VERSION (left-aligned).
    HeroInline,
}
    ...
    let mut result = match params.auth_state {
        ...
        AuthState::Done if params.is_zdr_blocked => {
            ...
            let (menu_rects, post_flush_escapes) = render_welcome_blocked(
                ...
                buf,
                Some((
                    "m0irai is not yet available for this account.",
                    theme.gray_bright,
                )),
                &menu,
                params.selected,
...
/// multi-line block showing the workspace path and the warning that m0irai
...
fn render_welcome_trust(
    ...
    let lines = vec![
        ...
        // joined into one, it runs past a compact viewport and truncates at
        // "...posing security risks".
        Line::from(Span::styled(
            "m0irai may run or modify contents in this directory,",
            Style::default().fg(theme.gray),
        ))
        .alignment(Alignment::Center),
        Line::from(Span::styled(
...
mod tests {
    ...
    fn version_badge_carries_no_release_label() {
        ...
        assert!(full.contains("m0irai"), "full badge: {full:?}");
        assert!(inline.contains("m0irai"), "inline badge: {inline:?}");
