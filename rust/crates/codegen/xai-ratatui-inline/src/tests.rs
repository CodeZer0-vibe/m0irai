...
mod links {
    ...
    #[test]
    fn invalidating_back_buffer_rewrites_blank_cells_over_stale_terminal_content() {
        let mut terminal = term(4, 1);
        assert_eq!(frame(&mut terminal, "MENU", &[]), "MENU");

        // The next target buffer is blank. A normal reset would also make the
        // comparison buffer blank and emit no bytes, leaving MENU physically
        // visible. Invalidation must instead force four blank-cell updates.
        terminal.invalidate_back_buffer();
        terminal.backend_mut().buf.clear();
        assert!(terminal.flush().unwrap());
        assert_eq!(terminal.backend().buf, b"    ");
    }
