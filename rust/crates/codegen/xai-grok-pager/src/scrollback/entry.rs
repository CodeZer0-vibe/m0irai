...
pub struct ScrollbackEntry {
    ...
    /// Optional presentation-owned cohesion group. Adjacent entries with the
    /// same non-zero group are rendered without an inter-entry spacer. This is
    /// deliberately separate from tool grouping/folding: room speakers use it
    /// to compose a header, activity rows, live status, and message body as one
    /// visual block without changing the pager's semantic block types.
    cohesion_group: Option<u64>,
...
impl ScrollbackEntry {
    ...
    pub fn ensure_cached(
        ...
        // UserPrompt, ToolCall, Thinking, BgTask, Subagent and the room's
        // steps block vary their output() based on is_selected — for all other
        // blocks the output is identical regardless of selection state.
        // Normalize to false for those blocks so selection changes don't thrash
        // the cache.
        //
        // The room's steps block is on this list because its collapsed row
        // carries the `(ctrl+e to expand)` affordance only while selected. A
        // block missing from this list reads `ctx.is_selected` correctly and is
        // handed `false` forever, with no compile error and no failing render —
        // which is why the proof for that hint is run against a tree WITHOUT
        // this line first.
    ...
    /// Attach this entry to a caller-owned visual cohesion group.
    pub(crate) fn with_cohesion_group(mut self, group: u64) -> Self {
        debug_assert!(group != 0, "cohesion group zero is reserved for no group");
        self.cohesion_group = Some(group);
        self
    }

    /// Whether this entry and `next` form one visually contiguous block.
    pub(crate) fn is_visually_contiguous_with(&self, next: &Self) -> bool {
        self.cohesion_group.is_some() && self.cohesion_group == next.cohesion_group
    }
    ...
    #[cfg(not(feature = "grok-runtime"))]
    fn rendered_output_with_hooks(&self, ctx: &BlockContext) -> RenderedBlockOutput {
        self.block.rendered_output(ctx)
    }
