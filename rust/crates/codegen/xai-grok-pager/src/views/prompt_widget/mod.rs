...
#[cfg(feature = "grok-runtime")]
type PromptChipElement = crate::app::agent::ChipElement;

#[cfg(not(feature = "grok-runtime"))]
#[derive(Debug, Clone)]
pub struct PromptChipElement {
...
#[cfg(feature = "grok-runtime")]
type ClearDetector = crate::tips::clear_detector::ClearDetector;
...
#[cfg(not(feature = "grok-runtime"))]
impl ClearDetector {
    fn observe_user_edit(&mut self, _before: usize, _after: usize) -> bool {
        false
    }
}
...
pub struct StashedPrompt {
    ...
    pub(crate) chip_elements: Vec<PromptChipElement>,
...
impl StashedPrompt {
    pub(crate) fn from_submission(
        ...
        chip_elements: Vec<PromptChipElement>,
    ) -> Self {
        Self {
            cursor: text.len(),
    ...
    pub(crate) fn into_submission(mut self) -> (String, Vec<PastedImage>, Vec<PromptChipElement>) {
        (
...
impl PromptWidget {
    ...
    pub fn slash_move_selection(&mut self, delta: isize) {
        ...
        #[cfg(not(feature = "grok-runtime"))]
        self.slash_controller
            .move_selection(&mut self.slash_state, delta);
    }
    ...
    pub fn slash_scroll_selection(&mut self, delta: isize) {
        ...
        #[cfg(not(feature = "grok-runtime"))]
        self.slash_controller
            .scroll_selection(&mut self.slash_state, delta);
    }
    ...
    /// Replace the room-facing slash catalog without coupling the composer to
    /// a service command registry. The caller owns command semantics; the
    /// widget owns matching, selection, and insertion.
    #[cfg(not(feature = "grok-runtime"))]
    pub fn set_frontend_command_catalog(
        &mut self,
        commands: impl IntoIterator<Item = crate::slash::FrontendCommand>,
    ) {
        self.slash_controller.set_frontend_catalog(commands);
        self.refresh_frontend_slash();
    }

    /// Refresh room slash completion from the current editable buffer.
    #[cfg(not(feature = "grok-runtime"))]
    pub fn refresh_frontend_slash(&mut self) {
        let text = self.textarea.text().to_owned();
        let cursor = self.textarea.cursor();
        self.slash_controller
            .refresh_frontend(&mut self.slash_state, &text, cursor);
    }

    /// Accept the selected room slash completion and refresh its snapshot.
    #[cfg(not(feature = "grok-runtime"))]
    pub fn accept_slash_completion(&mut self) -> bool {
        let snap = self.slash_state.snapshot();
        let Some(row) = snap.selection().cloned() else {
            return false;
        };
        let Some(range) = snap.command_range else {
            return false;
        };
        if !snap.cursor_in_command
            || range.end > self.textarea.text().len()
            || !self.textarea.text().is_char_boundary(range.start)
            || !self.textarea.text().is_char_boundary(range.end)
        {
            return false;
        }
        self.textarea.replace_range(range, &row.insert_text);
        self.slash_controller
            .record_command_use(&snap.query, row.command_name());
        self.refresh_frontend_slash();
        true
    }
...
impl PromptWidget {
    ...
    pub fn restore_chip_elements(&mut self, elems: &[PromptChipElement]) {
