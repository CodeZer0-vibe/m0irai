...
impl WriterSync {
    pub fn new() -> Self {
        Self {
            ...
            sends_in_flight: Arc::new(AtomicU64::new(0)),
    ...
    fn begin_send(&self) -> std::io::Result<WriterSendGuard<'_>> {
        // The double check and the panic fence use sequentially consistent
        // ordering. If shutdown wins before this send is admitted, the second
        // check rejects it. If admission wins, shutdown observes the in-flight
        // count and waits for the accepted payload.
        if !self.accepting.load(Ordering::SeqCst) {
            return Err(std::io::Error::new(
                std::io::ErrorKind::BrokenPipe,
                "terminal writer is shutting down",
            ));
        }
        ...
        Ok(WriterSendGuard { sync: self })
    }
    /// Prevent any later frame from being accepted. A producer already inside
    /// its admission fence is included in [`Self::wait_quiesced`].
    pub fn stop_accepting(&self) {
        self.accepting.store(false, Ordering::SeqCst);
    }
    pub fn backpressure_count(&self) -> u64 {
        self.backpressure_count.load(Ordering::Acquire)
    }
    ...
    fn is_quiesced(&self) -> bool {
        self.sends_in_flight.load(Ordering::SeqCst) == 0 && self.is_drained()
    }
    ...
    /// Wait until no producer is inside frame admission and every accepted
    /// payload has reached the terminal writer. Call [`Self::stop_accepting`]
    /// first to turn this into a stable teardown fence.
    pub fn wait_quiesced(&self, timeout: Duration) -> std::io::Result<WriterDrain> {
        let deadline = Instant::now() + timeout;
...
struct WriterSendGuard<'a> {
    sync: &'a WriterSync,
}

impl Drop for WriterSendGuard<'_> {
    fn drop(&mut self) {
        self.sync.sends_in_flight.fetch_sub(1, Ordering::SeqCst);
    }
}
...
///
/// This absorbs brief PTY back-pressure without stalling the tokio event loop.
/// If the terminal remains behind after the bounded queue fills, `flush()`
/// applies backpressure to the producer. That preserves ordered ratatui diffs
/// and places a fixed upper bound on queued frame memory.
pub struct WriterPayload {
    pub(crate) sequence: u64,
    pub(crate) data: Vec<u8>,
}
/// Maximum number of complete terminal diffs waiting behind the frame that is
/// currently being written. Ratatui payloads are ordered diffs, so dropping or
/// replacing a queued payload would corrupt the next frame. A bounded channel
/// therefore applies backpressure while preserving every accepted diff.
const WRITER_QUEUE_CAPACITY: usize = 2;
pub type WriterSender = mpsc::SyncSender<WriterPayload>;
...
impl Write for TermWriter {
    ...
    fn flush(&mut self) -> std::io::Result<()> {
        ...
        let _send_guard = self.sync.begin_send()?;
        let sequence = self.sync.reserve_sequence();
        let data = std::mem::take(&mut self.buf);
        let payload = WriterPayload { sequence, data };
        let sent = match self.tx.try_send(payload) {
            Ok(()) => true,
            Err(mpsc::TrySendError::Full(payload)) => {
                self.sync.mark_backpressured();
...
impl WriterThread {
    ...
    /// exited. The [`WriterSender`] must be dropped *before* calling this,
...
pub fn spawn_writer_thread() -> (
    ...
    let (tx, rx) = mpsc::sync_channel::<WriterPayload>(WRITER_QUEUE_CAPACITY);
    let (event_tx, event_rx) = tokio::sync::mpsc::unbounded_channel();
    ...
    let test_delay: Option<Duration> = None;
...
mod tests {
    ...
    #[test]
    fn frame_write_delay_is_compiled_only_for_test_support() {
        let source = include_str!("draw.rs");
        assert!(source.contains("#[cfg(any(test, feature = \"test-support\"))]"));
        assert!(source.contains("GROK_TEST_FRAME_WRITE_DELAY_MS"));
        assert!(source.contains("#[cfg(not(any(test, feature = \"test-support\")))]"));
    }
    #[test]
    ...
    fn writer_sync_rejects_multiple_live_producers() {
        let (tx, _rx) = mpsc::sync_channel::<WriterPayload>(WRITER_QUEUE_CAPACITY);
        let sync = WriterSync::new();
    ...
    #[test]
    fn writer_channel_applies_ordered_bounded_backpressure() {
        let (tx, rx) = mpsc::sync_channel::<WriterPayload>(WRITER_QUEUE_CAPACITY);
        let sync = WriterSync::new();
        let mut writer = TermWriter::new(tx, sync.clone()).expect("single test writer");

        for frame in [b"frame one".as_slice(), b"frame two".as_slice()] {
            writer.write_all(frame).expect("buffer frame");
            writer.flush().expect("fill bounded queue");
        }

        let (done_tx, done_rx) = mpsc::channel();
        let blocked = std::thread::spawn(move || {
            writer
                .write_all(b"frame three")
                .expect("buffer blocked frame");
        ...
        assert_eq!(rx.recv().expect("first queued frame").data, b"frame one");
        done_rx
            .recv_timeout(Duration::from_secs(1))
            .expect("third frame unblocks after capacity is released")
            .expect("third frame send succeeds");
        assert_eq!(rx.recv().expect("second queued frame").data, b"frame two");
        assert_eq!(rx.recv().expect("third queued frame").data, b"frame three");
        blocked.join().expect("blocked writer thread joins");
        assert_eq!(sync.queued(), 3);
        assert_eq!(sync.backpressure_count(), 1);
    }
    #[test]
    fn writer_shutdown_fence_rejects_late_frames_without_reserving_a_sequence() {
        let (tx, _rx) = mpsc::sync_channel::<WriterPayload>(WRITER_QUEUE_CAPACITY);
        let sync = WriterSync::new();
        let mut writer = TermWriter::new(tx, sync.clone()).expect("single test writer");
        writer.write_all(b"late frame").expect("buffer frame");

        sync.stop_accepting();

        let error = writer.flush().expect_err("shutdown rejects frame");
        assert_eq!(error.kind(), std::io::ErrorKind::BrokenPipe);
        assert_eq!(sync.queued(), 0);
        assert!(matches!(
            sync.wait_quiesced(Duration::from_millis(20)),
            Ok(WriterDrain::Drained)
        ));
    }
    #[test]
