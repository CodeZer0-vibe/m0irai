...
impl ScrollbackPane {
    ...
    fn render_tool_call_hover(
        ...
        let mut hoverable = matches!(entry.block, RenderBlock::Thinking(_));
        #[cfg(not(feature = "grok-runtime"))]
        let hoverable = matches!(entry.block, RenderBlock::Thinking(_));
        #[cfg(feature = "grok-runtime")]
        {
            hoverable |= matches!(
                entry.block,
                RenderBlock::ToolCall(_) | RenderBlock::BgTask(_) | RenderBlock::Subagent(_)
            );
        }
        if !hoverable {
            return;
    ...
    fn render_content(
            ...
            && let Some(selected_abs) = state.selected()
        {
            ...
            #[cfg(feature = "grok-runtime")]
            let mut is_header_style_block = state
                .entry(selected_abs)
                .is_some_and(|e| matches!(e.block, RenderBlock::Thinking(_)));
            #[cfg(not(feature = "grok-runtime"))]
            let is_header_style_block = state
                .entry(selected_abs)
                .is_some_and(|e| matches!(e.block, RenderBlock::Thinking(_)));
            #[cfg(feature = "grok-runtime")]
            {
                is_header_style_block |= state
                    .entry(selected_abs)
