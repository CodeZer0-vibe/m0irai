...
mod tests {
    ...
    // The helpers must agree with the active console policy. The policy itself
    // is exhaustively exercised below with injected host/terminal pairs; this
    // keeps these public-helper checks independent of the test runner's host.
    #[test]
    fn glyph_helpers_match_active_console_policy() {
        if is_legacy_windows_console() {
            assert_eq!(diamond_filled(), "\u{2666}");
            assert_eq!(diamond_hollow(), "\u{25CB}");
            assert_eq!(diamond_dotted(), "\u{2666}");
            assert_eq!(diamond_filled_char(), '\u{2666}');
            assert_eq!(diamond_hollow_char(), '\u{25CB}');
            assert_eq!(braille_spinner_frames()[0], "|");
            assert_eq!(dot_spinner_frames()[2], "\u{00b7}");
            assert_eq!(
                monitor_icon_frames(),
                ["\u{00b7}", "\u{25CB}", "\u{2022}", "\u{25CB}"]
            );
        } else {
    ...
    // A legacy console maps unsupported glyphs, while every other console
    // retains the zero-copy input. The policy decision is tested separately
    // with injected inputs below.
    #[test]
    fn legacy_glyph_fallback_matches_active_console_policy() {
        let glyph = legacy_glyph_fallback("\u{2713} Saved");
        if is_legacy_windows_console() {
            assert_eq!(glyph.as_ref(), "\u{221A} Saved");
            assert!(matches!(glyph, Cow::Owned(_)));
        } else {
            assert!(matches!(glyph, Cow::Borrowed("\u{2713} Saved")));
        }
    }

    #[test]
    fn sanitize_toast_message_borrows_when_clean_on_every_console() {
        assert!(matches!(
