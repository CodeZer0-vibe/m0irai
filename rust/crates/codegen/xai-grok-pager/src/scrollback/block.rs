...
use super::blocks::{
    AgentMessageBlock, BtwBlock, CreditLimitBlock, SystemMessageBlock, ThinkingBlock,
    UserPromptBlock,
};
#[cfg(feature = "grok-runtime")]
use super::blocks::{
    BgTaskBlock, ContextInfoBlock, EditToolCallBlock, ExecuteToolCallBlock, LineRange,
    ListDirToolCallBlock, OtherToolCallBlock, ReadToolCallBlock, SearchFileMatch,
    SearchToolCallBlock, SessionEvent, SessionEventBlock, SubagentBlock, SubagentBlockKind,
    ToolCallBlock, WorkflowBlock,
};
use super::types::{
    AccentStyle, BlockBackground, BlockContext, BlockLine, BlockOutput, DisplayMode,
    RenderedBlockOutput, Selectable, SelectionBoundaries, derive_selection_text,
};
...
pub trait BlockContent {
    ...
    #[cfg(not(feature = "grok-runtime"))]
    fn inline_media_placements(&self, _ctx: &BlockContext) -> Vec<AnchoredMedia> {
        Vec::new()
    }
...
pub struct StubBlock {
    pub text: String,
    /// Optional owned styled lines. `text` remains the copy/search source of
    /// truth; styling is presentation only.
    pub styled_lines: Option<Vec<Line<'static>>>,
    pub accent_color: ratatui::style::Color,
    /// Whether this metadata row owns an accent rail. Room operator headers
    /// intentionally opt out while their prompt body retains its blue rail.
    pub show_accent: bool,
    ...
    pub groupable: bool,
    /// Compact stubs intentionally omit vertical padding. This is opt-in so
    /// existing stub behavior retains its normal transcript spacing.
    pub compact: bool,
...
impl StubBlock {
    ...
    /// Create a compact non-groupable stub for one-line transcript metadata.
    pub fn compact_non_groupable(
    ...
    /// Create compact room metadata with multiple semantic colors while
    /// retaining one plain-text copy/search representation.
    pub fn styled_compact_non_groupable(
        text: impl Into<String>,
    ...
    pub fn without_accent(mut self) -> Self {
...
impl BlockContent for StubBlock {
    fn output(&self, _ctx: &BlockContext) -> BlockOutput {
        let mut output = self
            .styled_lines
            .as_ref()
            .map(|lines| BlockOutput {
                lines: lines.iter().cloned().map(BlockLine::styled).collect(),
            })
            .unwrap_or_else(|| BlockOutput::plain(&self.text));
...
pub enum RenderBlock {
    ...
    /// One room stream's folded tool steps. The room's own block, and the
    /// first one it builds that respects `DisplayMode`.
    #[cfg(feature = "room-runtime")]
    RoomSteps(crate::room_steps_block::RoomStepsBlock),
}
...
macro_rules! delegate_block {
    ($self:expr, $method:ident ( $($arg:expr),* )) => {
        match $self {
            ...
            #[cfg(feature = "room-runtime")]
            RenderBlock::RoomSteps(b) => b.$method($($arg),*),
        }
    };
}
...
impl RenderBlock {
    ...
    /// Create a compact non-groupable stub for one-line transcript metadata.
    pub fn stub_compact_non_groupable(
        text: impl Into<String>,
        accent_color: ratatui::style::Color,
    ) -> Self {
        RenderBlock::Stub(StubBlock::compact_non_groupable(text, accent_color))
    }

    /// Create a compact styled metadata block with a separate plain-text
    /// selection/search representation.
    pub fn stub_styled_compact_non_groupable(
        text: impl Into<String>,
        lines: Vec<Line<'static>>,
        accent_color: ratatui::style::Color,
    ) -> Self {
        RenderBlock::Stub(StubBlock::styled_compact_non_groupable(
            text,
            lines,
            accent_color,
        ))
    ...
    /// Create styled compact metadata without an accent rail.
    pub fn stub_styled_compact_non_groupable_without_accent(
        text: impl Into<String>,
        lines: Vec<Line<'static>>,
        accent_color: ratatui::style::Color,
    ) -> Self {
        RenderBlock::Stub(
            StubBlock::styled_compact_non_groupable(text, lines, accent_color).without_accent(),
        )
    }
    ...
    /// Create an agent message whose native entry rail is owned by a caller
    /// supplied, service-neutral speaker color.
    pub fn agent_message_with_accent(
        text: impl Into<String>,
        accent_color: ratatui::style::Color,
    ) -> Self {
        RenderBlock::AgentMessage(AgentMessageBlock::with_accent(text, accent_color))
    }

    /// FL-141: an agent message whose TEXT paints `dim_text` rather than the
    /// pager's default markdown foreground. The room uses it for a cancelled
    /// lane, where the rail and the text must both read gray - the rail alone
    /// is what the first fix shipped, and a review caught it on a real frame.
    pub fn agent_message_dimmed_with_accent(
        text: impl Into<String>,
        accent_color: ratatui::style::Color,
        dim_text: ratatui::style::Color,
    ) -> Self {
        RenderBlock::AgentMessage(AgentMessageBlock::dimmed_with_accent(
            text,
            accent_color,
            dim_text,
        ))
    ...
    /// Create a streaming agent message with a stable native accent rail.
    pub fn agent_message_streaming_with_accent(accent_color: ratatui::style::Color) -> Self {
        RenderBlock::AgentMessage(AgentMessageBlock::streaming_with_accent(accent_color))
    }
    ...
    /// Check if this block is the room's folded steps block.
    ///
    /// Read by `ScrollbackEntry::ensure_cached`, which otherwise normalizes
    /// `is_selected` to `false` for every block outside a named list — so
    /// without this the block would read `ctx.is_selected` correctly and receive
    /// `false` forever, and the selected-row expand hint could never render.
    #[cfg(feature = "room-runtime")]
    pub fn is_room_steps(&self) -> bool {
        matches!(self, RenderBlock::RoomSteps(_))
    }

    #[cfg(not(feature = "room-runtime"))]
    pub fn is_room_steps(&self) -> bool {
        false
    }
    ...
    pub fn accent_color(&self) -> Option<ratatui::style::Color> {
        ...
        match self {
            ...
            RenderBlock::Stub(block) => block.show_accent.then_some(block.accent_color),
            // Inert in this workspace: `grok-runtime` is a known-but-never-set
            // cfg value (`rust/Cargo.toml` check-cfg), so this arm cannot be
            // compiled here. It exists so the M2 tree that does set it is not
            // left with a non-exhaustive match.
            #[cfg(feature = "room-runtime")]
            RenderBlock::RoomSteps(block) => Some(block.accent_color()),
        }
    }
    ...
    pub fn searchable_text(&self) -> Option<String> {
        match self {
            ...
            // Inert here for the same reason as the `accent_color` arm above.
            #[cfg(feature = "room-runtime")]
            RenderBlock::RoomSteps(b) => join_searchable([Some(b.full_text())]),
        }
    }
...
impl RenderBlock {
    ...
    pub fn accent_color(&self) -> Option<ratatui::style::Color> {
        ...
        match self {
            ...
            RenderBlock::Btw(_) => Some(theme.accent_plan),
            RenderBlock::Stub(block) => block.show_accent.then_some(block.accent_color),
            RenderBlock::AgentMessage(_) | RenderBlock::System(_) | RenderBlock::CreditLimit(_) => {
                None
            }
            #[cfg(feature = "room-runtime")]
            RenderBlock::RoomSteps(block) => Some(block.accent_color()),
    ...
    pub fn has_normal_fullscreen_viewer(&self) -> bool {
        matches!(
            self,
            RenderBlock::AgentMessage(_) | RenderBlock::Thinking(_)
        )
    }

    /// The room's own `AgentMessage` is copyable, so "room content blocks carry
    /// copyable text" is an established local pattern rather than a new promise.
    /// `copy_text`'s `_ => None` default is the trap this arm exists to close:
    /// the first production copy surface would otherwise silently inherit a
    /// wrong answer with no compiler error. There is no such surface in the room
    /// today — the arm is written ahead of it deliberately.
    pub fn supports_copy(&self) -> bool {
        match self {
            RenderBlock::UserPrompt(_)
            | RenderBlock::AgentMessage(_)
            | RenderBlock::Thinking(_) => true,
            #[cfg(feature = "room-runtime")]
            RenderBlock::RoomSteps(_) => true,
            _ => false,
        }
    }
    ...
    pub fn copy_text(&self, raw: bool) -> Option<String> {
        match self {
            RenderBlock::UserPrompt(block) => Some(block.copy_text()),
            RenderBlock::AgentMessage(block) => Some(block.copy_text(raw)),
            RenderBlock::Thinking(block) => Some(block.copy_text(raw)),
            #[cfg(feature = "room-runtime")]
            RenderBlock::RoomSteps(block) => Some(block.full_text()),
            _ => None,
        }
    }
    ...
    pub fn searchable_text(&self) -> Option<String> {
        match self {
            RenderBlock::Stub(block) => join_searchable([Some(block.text.clone())]),
            RenderBlock::UserPrompt(block) => join_searchable([Some(block.text.clone())]),
            RenderBlock::AgentMessage(block) => join_searchable([Some(block.copy_text(false))]),
            RenderBlock::Thinking(block) => join_searchable([Some(block.copy_text(false))]),
            RenderBlock::System(block) => join_searchable([Some(block.text.clone())]),
            RenderBlock::Btw(block) => join_searchable([
                Some(block.question.clone()),
                Some(block.content().rendered_plain_text()),
            ]),
            RenderBlock::CreditLimit(block) => {
                join_searchable([Some(block.heading.clone()), Some(block.url.clone())])
            }
            #[cfg(feature = "room-runtime")]
            RenderBlock::RoomSteps(block) => join_searchable([Some(block.full_text())]),
        }
    }
