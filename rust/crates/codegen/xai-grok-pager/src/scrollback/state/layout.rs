...
impl ScrollbackState {
    ...
    /// Keep an armed anchor meaningful across a [`Self::move_group_to_end`]
    /// that follows it (slice D's cross-turn relocation): when the anchored
    /// entry was itself carried to the tail, re-point at whatever survivor
    /// shifted into the vacated index — the row the operator was reading is
    /// still there, only its index changed. When nothing survived at that
    /// index (the moved block reached the top of the list), drop the anchor
    /// and let the plain max-offset clamp take over. An anchor on a NON-moved
    /// entry survives untouched: `shift_remove` + tail reinsert preserves every
    /// survivor's relative order, so its identity still pins the same content.
    pub(super) fn migrate_structural_anchor_past_move(
        &mut self,
        moved: &[EntryId],
        vacated: usize,
    ) {
        ...
        if !moved.contains(&anchor.id) {
            return;
        }
        let survivor = self.entries.get_index(vacated).map(|(id, _)| *id);
        self.structural_scroll_anchor = survivor.map(|id| StructuralScrollAnchor {
            id,
            rows_into_span: 0,
            armed_scroll_offset: anchor.armed_scroll_offset,
    ...
    pub(super) fn extend_layout_cache_with_new_entry(&mut self, new_idx: usize) -> bool {
        ...
        {
            let visually_contiguous = prev_entry.is_visually_contiguous_with(new_entry);
            ...
            cache.entries[new_idx - 1].gap_after =
                if visually_contiguous || (both_groupable && both_collapsed) {
                    0
                } else {
                    1
                };
    ...
    fn recompute_gap_after(
        ...
        for (i, cached) in cached_entries.iter_mut().enumerate() {
            ...
            let visually_contiguous = a.is_visually_contiguous_with(b);
            ...
            cached.gap_after = if visually_contiguous || (both_groupable && both_collapsed) {
                0
            } else {
                1
            };
