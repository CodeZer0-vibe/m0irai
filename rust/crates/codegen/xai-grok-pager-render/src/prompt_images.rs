...
/// Frontend-owned MIME inference for local preview/render paths. Service
/// session/ACP serialization remains behind `grok-runtime` below.
...
pub fn decode_image_dimensions(bytes: &[u8]) -> Option<(u32, u32)> {
    // The renderer only needs the decoded dimensions; validation and tool
    // policy belong to the service runtime. `image` retains the same broad
    // format support for frontend previews without pulling that runtime in.
...
impl ImageViewerState {
    ...
    pub fn open_from_path(path: &std::path::Path) -> Option<Self> {
        ...
        let mime_type = mime_from_bytes(&bytes).to_owned();
...
fn read_image_at_path(path: &std::path::Path) -> Option<PastedImage> {
    ...
    let mime_type = mime_from_bytes(&data);
...
mod tests {
    ...
    #[cfg(windows)]
    #[test]
    fn try_read_image_with_quoted_spaces_and_parens() {
        let dir = tempfile::tempdir().unwrap();
        let real_path = dir.path().join("screenshot (2).png");
        std::fs::write(&real_path, make_test_png(10, 10)).unwrap();

        let pasted = format!("\"{}\"", real_path.display());
        let image = try_read_image_from_path(&pasted).expect("quoted native path");
        assert_eq!(image.source_path.as_deref(), Some(real_path.as_path()));
    }
    ...
    #[cfg(windows)]
    #[test]
    fn try_read_image_with_single_quoted_spaces() {
        let dir = tempfile::tempdir().unwrap();
        let real_path = dir.path().join("my file.png");
        std::fs::write(&real_path, make_test_png(10, 10)).unwrap();
    ...
    #[cfg(windows)]
    #[test]
    fn quoted_path_preserves_native_windows_separators() {
        let dir = tempfile::tempdir().unwrap();
        ...
        let image = try_read_image_from_path(&pasted).expect("quoted native path");
        assert_eq!(image.source_path.as_deref(), Some(path.as_path()));
    }
    ...
    fn file_url_with_localhost_host() {
        ...
        let mut pasted = url::Url::from_file_path(&p).expect("temp path file URL");
        pasted.set_host(Some("localhost")).unwrap();
        assert!(try_read_image_from_path(pasted.as_str()).is_some());
    }
    ...
    fn dropped_path_percent_encoded_question_round_trips() {
        ...
        let url = build_file_url(dir.path(), "query%3Ffile.txt");
    ...
    #[test]
    fn dropped_path_percent_encoded_question_round_trips() {
        // `?` is not a valid Windows filename byte, so exercise the URL
        // decoder without attempting to create such a file. An explicit
        // file URL still represents an intentional non-image drop and must
        // preserve the decoded literal rather than treating it as a query.
        let dir = tempfile::tempdir().unwrap();
        let expected = dir.path().join("query?file.txt");
        let url = build_file_url(dir.path(), "query%3Ffile.txt");

        assert_eq!(dropped_non_image_paths(&url), vec![expected]);
    }
    ...
    #[cfg(all(unix, feature = "grok-runtime"))]
    #[test]
