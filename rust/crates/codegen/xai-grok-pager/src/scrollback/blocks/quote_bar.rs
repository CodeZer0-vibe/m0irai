...
mod tests {
    ...
    // Every test in this module resolves the process-global theme, most of them
    // indirectly: `quote_bar_style()` reads `Theme::current().md_muted`, and the
    // strip/selection logic then matches spans against that exact style. Under the
    // terminal-native palette `md_muted` is `Reset`, so a theme flip between the
    // render pass and the comparison leaves the bar unrecognised and `│` in the
    // copy text. `blocks/thinking.rs`'s
    // `thinking_body_dim_italic_survives_the_terminal_native_palette` performs that
    // flip mid-run under `cache::test_lock()`; `pin_theme()` takes the same lock.
    // Measured: `wrapped_quote_continuations_exclude_reinjected_prefix` failed once
    // in 200 runs of this binary before the guards (m0irai Phase 4).
