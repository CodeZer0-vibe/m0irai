...
mod tests {
    ...
    fn file_target_provenance_survives_overlay_to_visible_map() {
        let path_buf = std::env::temp_dir()
            .join("non-display-target")
            .join("file name.rs");
        let expected_url = url::Url::from_file_path(&path_buf)
            .expect("platform temp path is absolute")
            .to_string();
        let path = Arc::<std::path::Path>::from(path_buf);
        let mut overlay = LinkOverlay::new();
        overlay.push(OverlayLink {
            screen_row: 3,
            col_start: 4,
        ...
        assert_eq!(resolved.osc8_url.unwrap().as_ref(), expected_url);
