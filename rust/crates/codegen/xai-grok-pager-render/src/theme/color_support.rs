...
mod tests {
    ...
    fn unknown_terminal_256color_fallback() -> ColorLevel {
        if cfg!(windows) {
            // Native Windows is documented as a TrueColor fallback even when
            // terminal environment variables only advertise 256 colors.
            ColorLevel::TrueColor
        } else {
            ColorLevel::Ansi256
        }
    ...
    fn standalone_color_uses_stderr_terminal_not_stdout() {
        ...
        assert_eq!(
            ...
            StandaloneColorEvidence::Available(unknown_terminal_256color_fallback())
        );
        assert_eq!(
            standalone_from_env(
                &env(&[("COLORTERM", "truecolor")]),
