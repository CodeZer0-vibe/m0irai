...
mod tests {
    ...
    // Every test below that resolves the theme -- directly via `Theme::current()`
    // or indirectly through a render pass -- holds `pin_theme()` for its whole body.
    // The theme is process-global (`xai-grok-pager-render/src/theme/cache.rs`:
    // `CURRENT` + `TERMINAL_NATIVE_LOCK`), and `blocks/thinking.rs`'s
    // `thinking_body_dim_italic_survives_the_terminal_native_palette` flips
    // `set_terminal_native_lock(true)` mid-run while holding `cache::test_lock()`.
    // A reader that does not take that same lock can build its spans under one
    // palette and assert against the other: measured 2 failures in 200 runs of
    // this binary before the guards (m0irai Phase 4).
