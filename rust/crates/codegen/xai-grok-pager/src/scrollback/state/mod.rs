...
impl ScrollbackState {
    ...
    /// Add a block as part of one caller-owned visual cohesion group.
    pub(crate) fn push_block_with_cohesion(
        &mut self,
        block: RenderBlock,
        cohesion_group: u64,
    ) -> EntryId {
        self.push(ScrollbackEntry::new(block).with_cohesion_group(cohesion_group))
    }
    ...
    pub fn insert_block_before(&mut self, anchor: EntryId, block: RenderBlock) -> EntryId {
        self.insert_block_before_entry(anchor, ScrollbackEntry::new(block))
    }

    /// Insert a finalized block into an existing visual cohesion group.
    pub(crate) fn insert_block_before_with_cohesion(
        &mut self,
        anchor: EntryId,
        block: RenderBlock,
        cohesion_group: u64,
    ) -> EntryId {
        self.insert_block_before_entry(
            anchor,
            ScrollbackEntry::new(block).with_cohesion_group(cohesion_group),
        )
    }

    fn insert_block_before_entry(
        &mut self,
        anchor: EntryId,
        mut entry: ScrollbackEntry,
    ) -> EntryId {
    ...
    /// Move a group of entries to the tail of the scrollback, keeping their
    /// relative order and their identities (slice D's cross-turn relocation
    /// primitive; see `RoomScrollback::relocate_if_displaced`).
    ///
    /// `ids` is the group in the order it should read at the tail. Ids listed
    /// in `removable` may have been removed since the caller recorded them and
    /// are silently skipped; any OTHER missing id is caller drift and trips a
    /// debug assertion rather than moving a partial block.
    ///
    /// Returns `false` without mutating anything when the ids already form the
    /// exact tail in the requested order — the common shape of a repeated
    /// relocation, which must not re-arm anchors or dirty layout for nothing.
    ///
    /// Steps, per the slice-D design:
    ///   1. resolve + dedupe in caller order (removable members may be gone),
    ///   2. no-op when already the exact tail,
    ///   3. record selection by id — whole entries are carried across the
    ///      move, so display mode, pin, cohesion group and render cache ride
    ///      along by identity and need no separate bookkeeping,
    ///   4. require the moved ids to be uncommitted to native scrollback,
    ///   5. arm the structural viewport anchor,
    ///   6. `shift_remove` every member, then reinsert the same entries at the
    ///      tail in the supplied order,
    ///   7. migrate the armed anchor past the move,
    ///   8. re-resolve selection by id,
    ///   9. rewind the commit scan cursor to the vacated block top — a
    ///      length-only clamp is NOT enough, because non-moved entries above
    ///      the block shift up and could end up stranded below the cursor,
    ///   10. rebuild turns, dirty gaps, invalidate layout, bump content
    ///       generation (respecting batch mode).
    pub fn move_group_to_end(&mut self, ids: &[EntryId], removable: &[EntryId]) -> bool {
        let mut ordered: Vec<EntryId> = Vec::with_capacity(ids.len());
        for id in ids {
            ...
            if self.entries.contains_key(id) {
                ...
                debug_assert!(
                    ...
                    "move_group_to_end: member {id:?} is missing and not declared removable"
                );
            }
        }
        let Some(first_moved_index) = ordered
            .iter()
        ...
        // Step 2: exact-tail detection, before any mutation.
        let tail_start = match self.entries.len().checked_sub(ordered.len()) {
            Some(start) => start,
            None => return false,
        };
        let already_tail = ordered.iter().enumerate().all(|(offset, id)| {
            self.entries
                .get_index(tail_start + offset)
                .is_some_and(|(present, _)| present == id)
        });
        if already_tail {
            return false;
        }

        // Step 3: selection is an index today; keep the identity it points at.
        let selected_id = self
            .selected
            .and_then(|index| self.entries.get_index(index))
            .map(|(id, _)| *id);
        // Step 4: committed terminal text cannot be reordered — text that has
        // already been printed into the terminal's own scrollback cannot be
        // un-printed, so moving such an entry would leave the feed and the
        // terminal disagreeing about what the operator read.
        //
        // A REFUSAL and not only an assertion, decided rather than inherited.
        // The room cannot reach this state: `committed` is populated only by
        // `mark_committed`, which is reached only from `crate::minimal`'s
        // commit pipeline, and nothing in `room_scrollback.rs`, `room_view.rs`
        // or `app/room_runtime.rs` references that module. But this primitive
        // lives on the SHARED `ScrollbackState`, where minimal mode does run,
        // and a `debug_assert` alone would let a future caller there corrupt
        // native scrollback in a release build while passing every test. The
        // check is one lookup per moved id against a set that is empty on the
        // only path that calls this today.
        //
        // The `debug_assert` stays beside it so the mistake is LOUD in a test
        // run rather than silently doing nothing.
        let already_printed = ordered.iter().any(|id| self.committed.contains(id));
        debug_assert!(
            !already_printed,
            "move_group_to_end: a moved entry is already committed to native scrollback"
        );
        if already_printed {
            return false;
        }

        // Step 5: anchor the viewport top before indices shift.
        self.arm_structural_scroll_anchor();

        // Step 6: out, then back in, same values, same ids, tail position.
        // The id-keyed sibling sets (`running`, `dirty_heights`, `committed`,
        // `expanded_groups`) survive untouched because they key on EntryId,
        // never on position.
        let mut carried: Vec<(EntryId, ScrollbackEntry)> = Vec::with_capacity(ordered.len());
        for id in &ordered {
            if let Some(entry) = self.entries.shift_remove(id) {
                carried.push((*id, entry));
            }
        }
        for (id, entry) in carried {
            self.entries.insert(id, entry);
        }

        // Steps 7-8.
        self.migrate_structural_anchor_past_move(&ordered, first_moved_index);
        self.selected = selected_id.and_then(|id| self.index_of_id(id));
        // Step 9.
        self.commit_scan_cursor = self.commit_scan_cursor.min(first_moved_index);
    ...
    /// Replace an agent-message entry in place while preserving its stable
    /// identity, selection, and chronological position. A completed transport
    /// message is authoritative over its streamed preview, so it replaces the
    /// preview rather than creating a second chronological entry.
    pub fn replace_agent_message(&mut self, id: EntryId, block: RenderBlock) -> bool {
        if !matches!(block, RenderBlock::AgentMessage(_)) {
            return false;
        }
        let Some(entry) = self.entries.get_mut(&id) else {
            return false;
        };
        if !matches!(entry.block, RenderBlock::AgentMessage(_)) {
            return false;
        }
        entry.block = block;
        entry.invalidate_cache();
    ...
    /// Replace one room-owned compact status row in place. This deliberately
    /// accepts only Stub-to-Stub replacement so provider presentation cannot
    /// mutate arbitrary Grok scrollback block kinds.
    #[cfg(feature = "room-runtime")]
    pub fn replace_room_status(&mut self, id: EntryId, block: RenderBlock) -> bool {
        if !matches!(block, RenderBlock::Stub(_)) {
            return false;
        }
        let Some(entry) = self.entries.get_mut(&id) else {
            return false;
        };
        if !matches!(entry.block, RenderBlock::Stub(_)) {
            return false;
        }
        entry.block = block;
        entry.invalidate_cache();
    ...
    /// Replace one room-owned steps block in place. A copy of
    /// [`Self::replace_room_status`] with the type guard changed on both sides:
    /// the room may mutate its own folded step list and nothing else, so
    /// provider presentation still cannot reach an arbitrary Grok block kind.
    ///
    /// In place rather than remove-and-insert: `remove_entry` arms the
    /// structural scroll anchor on every call, so a burst of tool steps under a
    /// scrolled-up reader would be a burst of anchor arming. This goes through
    /// `dirty_heights`, which `prepare_layout` handles on its incremental path,
    /// and it preserves the entry's id AND its `display_mode` — which is what
    /// keeps an operator's manual expand alive across the lane ending.
    #[cfg(feature = "room-runtime")]
    pub fn replace_room_steps(&mut self, id: EntryId, block: RenderBlock) -> bool {
        if !matches!(block, RenderBlock::RoomSteps(_)) {
            return false;
        }
        let Some(entry) = self.entries.get_mut(&id) else {
            return false;
        };
        if !matches!(entry.block, RenderBlock::RoomSteps(_)) {
            return false;
        }
        entry.block = block;
        entry.invalidate_cache();
    ...
    /// Whether ANY of `ids` has already been emitted into native scrollback.
    ///
    /// For callers that must decide BEFORE they start writing whether a group
    /// can be reordered at all — [`Self::move_group_to_end`] refuses such a
    /// group, and a caller that had already inserted a row above it by then
    /// would be left with that row explaining a move that did not happen.
    pub(crate) fn any_committed(&self, ids: &[EntryId]) -> bool {
        ids.iter().any(|id| self.committed.contains(id))
    }
...
/// [`ScrollbackState::move_group_to_end`]'s own tests, under a plain
/// `#[cfg(test)]` gate on purpose.
///
/// This file's main `mod tests` sits behind `feature = "grok-runtime"`, and no
/// crate in this vendored workspace declares that feature — the pager's
/// runtime modules were not vendored (see this crate's Cargo.toml header), so
/// tests parked there never compile HERE and a green suite would be filtering
/// them out, not passing them. Slice D's relocation primitive is new surface;
/// its tests must actually run, so they live here.
#[cfg(test)]
mod move_group_tests {
    use super::*;

    fn stub(text: &str) -> RenderBlock {
        RenderBlock::stub(text, ratatui::style::Color::Blue)
    }

    #[test]
    fn move_group_to_end_carries_a_middle_block_to_the_tail_in_order() {
        // A weaker assertion (members present, count unchanged) would pass an
        // implementation that APPENDS copies and leaves the originals behind -
        // the duplicate-lane-rows failure slice D exists to kill.
        let mut state = ScrollbackState::new();
        let a = state.push_block(stub("a"));
        let b = state.push_block(stub("b"));
        let c = state.push_block(stub("c"));
        let d = state.push_block(stub("d"));

        assert!(state.move_group_to_end(&[b, c], &[]));

        assert_eq!(state.len(), 4);
        assert_eq!(state.index_of_id(a), Some(0));
        assert_eq!(state.index_of_id(d), Some(1));
        assert_eq!(state.index_of_id(b), Some(2));
        assert_eq!(state.index_of_id(c), Some(3));
    }

    #[test]
    fn move_group_to_end_is_a_no_op_on_the_exact_tail() {
        // Relocation runs on every qualifying commit; a repeated commit into
        // an already-tail block must not re-arm anchors or dirty layout.
        let mut state = ScrollbackState::new();
        let a = state.push_block(stub("a"));
        let b = state.push_block(stub("b"));
        let c = state.push_block(stub("c"));

        // b, c are already the exact tail in order.
        assert!(!state.move_group_to_end(&[b, c], &[]));
        assert_eq!(state.index_of_id(b), Some(1));
        assert_eq!(state.index_of_id(c), Some(2));

        // A genuine move, then its own repeat: true once, false forever after.
        assert!(state.move_group_to_end(&[a, b], &[]));
        assert!(!state.move_group_to_end(&[a, b], &[]));
        assert_eq!(state.index_of_id(c), Some(0));
        assert_eq!(state.index_of_id(a), Some(1));
        assert_eq!(state.index_of_id(b), Some(2));
    }

    #[test]
    fn move_group_to_end_keeps_selection_running_flag_and_fold_by_identity() {
        // A weaker test that checks only entry ORDER would pass an
        // implementation that re-CREATES the entries at the tail: new ids
        // would strand the room's stable lookups, a dropped running flag
        // would freeze a live stream's spinner, and a reset fold mode would
        // pop an operator-expanded steps group shut mid-read.
        let mut state = ScrollbackState::new();
        let a = state.push_block(stub("a"));
        let b = state.push_block(stub("b"));
        state.set_entry_running(b, true);
        state
            .get_by_id_mut(a)
            .unwrap()
            .set_display_mode(DisplayMode::Expanded);
        ...
        assert!(state.move_group_to_end(&[a], &[]));

        assert_eq!(state.index_of_id(b), Some(0));
        assert_eq!(state.index_of_id(a), Some(1));
        // Selection kept its ENTRY (b), not its index: b slid up to 0.
        assert_eq!(state.selected(), Some(0));
        let untouched = state.get_by_id(b).unwrap();
        assert!(untouched.is_running);
        let moved = state.get_by_id(a).unwrap();
        assert_eq!(moved.display_mode, DisplayMode::Expanded);
    }

    #[test]
    fn move_group_to_end_never_strands_an_uncommitted_entry_below_the_commit_frontier() {
        // Dead D.4 clamped the cursor by LENGTH only; moving a MIDDLE block
        // shifts its unmoved neighbours UP, so a length clamp leaves the
        // cursor above them and those entries vanish from both the native
        // scan and the live tail. Under a LEGAL cursor (nothing uncommitted
        // below it) the prescribed rewind `min(old, first_moved)` is usually
        // a no-op - the moved ids are uncommitted, so the block starts at or
        // below the cursor anyway - which is exactly why the formula needs a
        // pin: a "simpler" length clamp passes every happy-path test and
        // strands survivors the moment some other path leaves the cursor
        // somewhere unusual.
        let mut state = ScrollbackState::new();
        state.push_block(stub("a"));
        let b = state.push_block(stub("b"));
        let c = state.push_block(stub("c"));
        state.set_commit_scan_cursor(3); // parked past everything

        assert!(state.move_group_to_end(&[b], &[]));

        assert_eq!(
            state.commit_scan_cursor(),
            1,
            "rewind to the vacated block top (b sat at index 1)"
        );
        assert_eq!(state.index_of_id(c), Some(1));
        assert_eq!(state.index_of_id(b), Some(2));
    }

    #[test]
    fn move_group_to_end_skips_a_declared_removable_member() {
        // The room clears a lane's live-status row before relocating its turn,
        // so a membership snapshot can legitimately name a removed id. It must
        // be skipped, not fatal and not resurrected.
        let mut state = ScrollbackState::new();
        state.push_block(stub("a"));
        let gone = state.push_block(stub("gone"));
        let c = state.push_block(stub("c"));
        let d = state.push_block(stub("d"));
        assert!(state.remove_entry(gone));

        assert!(state.move_group_to_end(&[gone, c], &[gone]));
    ...
    #[test]
    #[should_panic(expected = "not declared removable")]
    fn move_group_to_end_debug_asserts_on_a_missing_permanent_member() {
        // Registry drift - a writer that removed a lane row without updating
        // the turn-block registry - must fail loudly in debug builds instead
        // of silently moving a PARTIAL block and splitting one turn across
        // the scrollback.
        let mut state = ScrollbackState::new();
        state.push_block(stub("a"));
        let ghost = EntryId::new(99_999);
        state.move_group_to_end(&[ghost], &[]);
    }
}
