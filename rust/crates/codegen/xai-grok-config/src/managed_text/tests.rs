//! Tests for `managed_text`.
//!
//! **K3 (m0irai Phase 4, 2026-08-17).** Eleven of these are `ignore`d on Windows
//! because the module itself is broken there at this pin - not because the tests
//! are flaky. Two distinct upstream defects, both measured on
//! windows-x86_64 / rustc 1.94.0:
//!
//! 1. `source::FileIdentity` is `(dev, ino)` on unix but `(len, modified)` on
//!    every other platform, and it is applied to the target's PARENT DIRECTORY.
//!    NTFS bumps a directory's mtime whenever a child is created or removed -
//!    including the `grok.lock` file the transaction itself creates - so
//!    `ensure_and_anchor`/`revalidate` see a changed identity and every
//!    `apply()` fails `ParentChanged`. A mutable attribute is being used as an
//!    identity; the Windows analogue of an inode is volume-serial + file-index
//!    read from an open handle.
//! 2. `invalid_inputs_and_all_marker_shapes_are_refused` additionally expects
//!    `UnsafePath` for a unix-shaped absolute literal that Windows does not
//!    treat as absolute.
//!
//! `transaction_lock_blocks_second_apply_then_stale_revalidation_wins` does not
//! merely fail: defect 1 makes `apply_with_observer` return before the
//! `AfterLock` phase, so the test's `Barrier::new(2)` is never released and the
//! process HANGS forever. That is why it is ignored rather than left red.
//!
//! Nothing in the m0irai room reaches this module: its only consumer is
//! `xai-grok-pager::diagnostics::fix`, behind the `grok-runtime` feature that
//! Phase 4 removed. These tests still run (and must pass) on unix CI.
...
#[cfg_attr(
    windows,
    ignore = "K3: managed_text does not work on Windows at this pin - see the module header"
)]
